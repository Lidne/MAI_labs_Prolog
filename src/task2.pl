...
student_grades(Student, Grades) :-
    findall(Grade, (subject(_, GradesList), member(grade(Student, Grade), GradesList)), Grades).

student_avg_and_pass(Student, Avg, Passed) :-
    student_grades(Student, Grades),
    sum_list(Grades, Sum),
    length(Grades, Count),
    Count > 0,  % студент сдавал экзамены
    Avg is Sum / Count,
    (member(2, Grades) -> Passed = 'Failed'; Passed = 'Passed').

failed_students_count(Subject, Count) :-
    subject(Subject, GradesList),
    findall(Student, member(grade(Student, 2), GradesList), FailedStudents),
    length(FailedStudents, Count).

group_max_avg_student(Group, MaxAvg, Students) :-
    group(Group, StudentList),
    % Найти средний балл каждого студента в группе
    findall((Student, Avg), (member(Student, StudentList), student_avg_and_pass(Student, Avg, _)), StudentAvgs),
    % Найти максимальный средний балл
    findall(Avg, member((_, Avg), StudentAvgs), Avgs),
